# SOXL Bots — Retired, Archived Reference

**Status: retired.** These two bots (`soxl_bot.py` swing, `soxl_intraday_bot.py`
intraday) have been removed from the live Stock-bot system — no cron
entries, not deployed, not actively maintained. This document is a
preserved reference, split out from the main `README.md` so the active
documentation only covers what's actually running.

**Why preserved rather than deleted entirely:** both strategies went
through real, validated backtesting (5-year walk-forward for the swing
bot, 60-day for intraday) — genuine research worth keeping on record in
case a similar leveraged-ETF strategy is ever revisited, even though
neither bot is currently running.

**If genuinely reviving either bot**: treat this document as a starting
reference, not a guarantee of current accuracy — validate parameters
and dependencies fresh rather than assuming nothing has drifted since
this was archived.
...
## Retired: SOXL Cron Schedule (was PAUSED, now removed from live crontab)
...
## Retired: SOXL Bot Summary Rows

| Bot | File | Ticker | Data | Schedule |
|---|---|---|---|---|
