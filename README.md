...
Wired into `tqqq_buy_ladder_bot.py`'s `/marketcheck` and `/buyfilled`
embeds (a `📅 {date}` field showing today's flags, with a ⚠️ if
`is_high_impact_day`), and into `ladder_log.jsonl` (`event_flags_date` /
`event_flags_summary` on every logged fill/check) — so past fills can
later be cross-referenced against whether they landed on a flagged day,
without hand-matching dates.
...
*Last updated: September 2026 — Added `event_calendar_bot.py` +
...
fetch (now preserved per-category). Wired into `tqqq_buy_ladder_bot.py`'s
`/marketcheck` and `/buyfilled` embeds and into `ladder_log.jsonl`.
