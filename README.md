...
mean-reversion approach. Buys a brief pullback to VWAP or the 9 EMA *within* an
...
2. EMA9 > EMA13                          (short-term uptrend confirmed)
...
4. prev bar low within $0.75 of VWAP     (the pullback — dip toward support)
   OR within $0.75 of EMA9
5. close >= VWAP AND close >= EMA9       (the recovery — dip was bought)
```
...
EMA9/EMA13/volume baseline to populate) or after 3:30pm (not enough time left
...
**Full stats at VOLUME_MULT=1.2** were not independently re-run as a
standalone check (only the sweep summary and a full stats run at 1.3x were
completed) — worth confirming with a dedicated Cell-5-equivalent run
(exit breakdown, per-year table, equity curve) before treating 1.2x's
numbers as fully validated on their own, rather than inferred from the
sweep table alone.
...
independently converges on established practice rather than contradicting it.
...
**Important caveat:** this sweep was run using the **original (flawed)**
rolling-20-bar volume methodology, before the volume methodology correction
above. `PULLBACK_DIST` and `VOLUME_MULT` have never been jointly re-swept
together under corrected methodology — each was validated independently, at
different points in the process, against a different volume baseline.
The live bot currently runs both `PULLBACK_DIST=0.75` and `VOLUME_MULT=1.2`
simultaneously, but this exact combination has not itself been backtested
as a unit. A combined 2D sweep (pullback × corrected volume) is a reasonable
next validation step, not yet completed.
...
```json
{
  ...
  "ema_slow":         80.10,
  "vol_mult":         1.7,
  "pullback_src":     "VWAP + EMA9", ← which reference the pullback was near
  "prev_low":         80.10,
  "prev_vwap":        80.05,
  "prev_ema9":        80.12,
  "ema_spread_cur":     0.10,        ← EMA9-EMA13 spread, current bar (logged only, not on alert)
...
- **EMA spread trend (accelerating/decelerating)** — computed and logged
  ...
  the spread (as opposed to the EMA9>EMA13 threshold itself, which is
...
pullback toward VWAP/EMA9) has nothing to trigger on, since there's no dip to
...
*Last updated: July 2026 — corrected TQQQ intraday volume baseline methodology
(backtest now matches live: time-of-day, prior-days-only, no lookahead bias),
re-validated VOLUME_MULT (1.0→1.2) and PULLBACK_DIST (0.50→0.75) against
corrected methodology, added live alert context (VWAP extension, time-until-
cutoff, earlier-signal status), fixed off-by-one bugs in reconcile() and
check_earlier_signals_status(), widened yfinance fetch to 7d (Yahoo's true
limit), fixed volume-baseline self-referencing bias*
