...
3. volume >= 1.0x time-of-day average    (real participation, not drift)
4. prev bar low within $0.50 of VWAP     (the pullback — dip toward support)
   OR within $0.50 of EMA9
...
**Per-year consistency** (win rate never dropped below 63% in any year,
...
#### Why VOLUME_MULT = 1.0, Not 1.5

This was the most counter-intuitive finding. The original assumption — higher
volume threshold = higher conviction = better trades — was backwards for this
specific pattern. A full sensitivity sweep from 1.0x to 3.0x showed win rate
and expectancy **monotonically decreasing** as the volume threshold increased:
```
1.0x: 651 trades, 65.4% WR, $0.173 expectancy  ← selected
1.2x: 518 trades, 63.9% WR, $0.159 expectancy
1.5x: 353 trades, 61.5% WR, $0.139 expectancy  (original assumption)
1.8x: 224 trades, 59.8% WR, $0.123 expectancy
2.0x: 162 trades, 58.6% WR, $0.114 expectancy
```
Reasoning: the pullback pattern this bot trades is a quiet, orderly dip and
recovery — not a violent volume spike. Requiring elevated volume was
inadvertently filtering for choppier, less reliable setups, and filtering out
the clean, low-drama pullbacks that actually work.
...
each one:
1. Re-fetches 5 days of 1-min data (same `yfinance` call as the live bot)
2. Locates the entry point: the first 1-min bar strictly after the signal's
   15-min bar closes (`signal_bar_ts + 15min`) — this exactly matches the
   entry-timing assumption used in the original backtest
...
*Last updated: July 2026 — added tqqq_intraday_bot.py (momentum pullback,
15-min bars, automatic reconciliation, shared heartbeat, crash alerting)*
