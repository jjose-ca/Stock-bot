...
*Last updated: July 2026 — added structured rejection logging (which
condition failed, how close it was, normalized so higher %-of-required
always means "closer to passing" across every condition — fixed a direction
bug in the pullback condition's calculation), added automatic near-miss
reconciliation (hypothetical outcomes for close rejections, answering
whether current thresholds are correctly filtering or costing real trades),
fixed a gap-through-target/stop mispricing bug shared by real-trade and
near-miss reconciliation, extended `.gitattributes` pull-protection to cover
