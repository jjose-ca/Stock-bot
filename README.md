...
- **All five cron bots** (`tqqq_bot.py`, `tqqq_above_open_bot.py`, the
  SOXL bots, `event_calendar_bot.py`) and **all Colab research scripts**
  still fetch exclusively through `yfinance` — unchanged.
...
2. **`is_regular_market_hours_live`'s SPY holiday probe** — kept for
   ...
   bot suite (`tqqq_bot.py`, `soxl_intraday_bot.py`); low-frequency,
   low-stakes, fails open regardless of source.
...
- **`is_regular_market_hours_live()`** — combines a cheap 9:30-4:00 ET
  ...
  `is_market_open_today()`, itself mirroring `soxl_intraday_bot.py` — no
  hardcoded holiday calendar, infers "is today a trading day" from
  whether the data exists) to also catch holidays. Clock check runs
  first, so nights/weekends short-circuit before ever touching the
  network. Fails OPEN on probe error, matching the precedent bot's exact
  reasoning.
...
(`read_position_state() is None`) → fetch price
...
persisted) — resets on a bot restart. Deliberate: unlike
`position_state.json` (real financial data, must survive restarts),
losing this just means the tracker re-initializes from whatever price it
next sees — a minor, self-correcting inconvenience.
...
### Backtest Scripts — Entry-Threshold Research (Colab-Run, Not Deployed)
...
- **No automated daily/self-updating check — RESOLVED for the "no
  position" case, still open for "in a position."** A scheduled daily
  post was deferred pending a persistent position store; that store now
  exists (`position_state.json`, see its own section above) and powers
  the automatic dip-entry alert. What's still NOT built: any equivalent
  automatic check *while already holding a position* (e.g. watching for
  price approaching an already-computed Level 2/3) — this was explicitly
  scoped out as a different mechanism (watching a specific computed
  price vs. a running-high % dip), not a natural extension of the
  flat-position alert.
...
*Last updated: September 2026 (Session 3) — Migrated `tqqq_buy_ladder_bot.py`'s
