...
Discord-alert system for manual trading. Signals only — no automated order placement.
Five independent bots covering TQQQ (swing), TQQQ (intraday pullback —
**see status warning below**), TQQQ (intraday above-open — currently the
recommended TQQQ intraday strategy), SOXL (swing), and SOXL (intraday).
...
32 16 * * 1-5               tqqq_above_open_bot.py --reconcile  → tqqq_above_open.log
...
acted on, so they aren't silently forgotten:
...
*Last updated: July 2026 — MAJOR: found and fixed a critical lookahead-bias
bug in the backtest (entry was computed from a 15-min candle's LEFT-LABEL
timestamp instead of its actual close time, entering trades ~14 minutes
before the signal was really knowable). This retracted tqqq_intraday_bot.py's
(pullback strategy) validated edge — corrected, honest expectancy is
~$0.00/share, and the strategy loses to a plain random-entry baseline.
Built and validated tqqq_above_open_bot.py from scratch under corrected
methodology as the new recommended TQQQ intraday strategy (858 trades,
65.4% WR, $0.1767/share expectancy, tight $0.0012 out-of-sample gap) —
found via ablation-testing the retracted strategy's individual conditions.
Tested and rejected volume confirmation, EMA trend confirmation, and ORB as
enhancements to the new bot (all underperformed); validated a stop-streak
circuit breaker (not yet deployed) after verifying it was NOT itself
lookahead-biased (tested head-to-head against an independent
implementation on hand-verifiable data). Deployed the new bot to the VPS;
diagnosed and fixed a real crontab corruption incident (a merged-line bug
silently broke CRON_TZ for the whole file; a stale-daemon-reload race
caused temporary duplicate alerts/log entries) and manually deduplicated
the affected trade log data. Paused both SOXL bots. Above-open bot's
TRADE_START_M updated 30→0 after direct testing showed including the
10:00-10:30 window improves results, not degrades them. See
"Known Gaps" section for validated-but-not-yet-deployed items.
